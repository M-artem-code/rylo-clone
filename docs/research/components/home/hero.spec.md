...
page: /
screenshot: docs/design-references/rylo.com/hero-pc.png
...
## States & Behaviors
- **CTA Hover / Focus:** Button rolling label slides upward on hover (`group-hover:-translate-y-full`) with ease-cubic-button transition.
- **Scroll Parallax:** Background and foreground elements gently translate on scroll with CSS transform.

## Per-State Content
N/A — static content with hover interaction on CTA button
...
## Responsive Behavior
- phone 390: section 390×929px; heading 37.75px; body 15.0619px; 3 cols (4 items, gap normal); lh 19.5804px
- ipad 768: section 768×1227px; heading 47.2px; body 15.3994px; 3 cols (4 items, gap normal); lh 20.0193px
- pc 1440: section 1440×1361px; heading 64px; body 15.9995px; 3 cols (4 items, gap normal); lh 20.7994px
- change: heading size: 64px (pc) → 37.75px (phone)
- ipad matches pc layout
- exact per-property values: probe-hero.json + responsive.json (ground truth)

## Notes
- Extract data to `src/data/hero.ts`
- Use the downloaded hero background and foreground portrait images from `public/images/`
- Render the wave line and speech bubble badges ("Good morning", "It’s so wonderful to talk") positioned over the curve
- Implement the rating badge (4.6 stars, (3,574) count, Certified by FCC, 100% Private)
