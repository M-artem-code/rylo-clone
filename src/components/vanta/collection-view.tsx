        ...
        <div>
          ...
          <Reveal>
            <h1 className="mt-3 font-display text-[56px] leading-[0.95] text-milk md:text-[68px]">
              <HeadlineLines lines={collectionPage.headline} />
            </h1>
          </Reveal>
        ...
        <div className="flex flex-col">
          {collectionPage.rows.map((row) => (
            <Link
              key={row.name}
              href={row.href}
              className="vanta-row grid grid-cols-[200px_1fr] items-center gap-6 border-t border-graph py-4 last:border-b"
            >
              <CoverImage
                src={row.image}
                alt={row.name}
                className={row.name === "VOID" ? "vanta-share-void h-[150px]" : "h-[150px]"}
                position="center"
              />
              <div>
                <p className="vanta-row-name font-italiana text-[34px] tracking-[0.16em] text-milk">
                  {row.name}
                </p>
